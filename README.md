...
When attempting to interpret the data provided by the Analyzer, it is important to recognize which Category a given readout is coming from. An average of ~2ms in the Tick category, when adjusted for higher speeds, ```is worse than an average of ~3ms in the category section```. <!-- THIS IS UNCLEAR -->
...
```
After modification: <!-- Unclear what "modification" was performed? Sure it's a tech thing that might be going over my head, but even as a tutorial, I'm not clear on what happened. -->
```csharp
public static int MyTargetMethod(int param1, bool param2)
{
    // ...
...
Transpiler profiling is done using a relatively simple approach. The original methods <!-- method, singular, or is methods correct? --> IL is compared to the current IL (after all transpilers have been applied) and a diff algorithm is applied. 
...
### IEnumerable
If you are seeing spikes from relatively simple Postfixes which deal with IEnumerables, keep in mind how they work. <!-- What is "they"; the "relatively simple Postfixes", or "IEnumerables"? My guess is on "IEnumerables" but it's vague. --> This section is only relevant when the IEnumerable is being frontloaded. If you are doing a Postfix where a parameter you are checking is an IEnumerable, and you are doing a call like `.Any()` or `.Select`, your Postfix will force the calculation for the enumerable. <!-- IEnumberable? --> The time this takes will be attributed to your Postfix. Additionally, each IEnumerator generated from the IEnumerable will have to repeat the calculation. This is something you can avoid.
